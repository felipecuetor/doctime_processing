...
with open('../../../data/dblp_output.csv', 'w') as file:
...
for href in all_urls[1:]:
    try:
        driver.get(href)
        time.sleep(1)
        total_conference_list = driver.find_elements_by_class_name("entry")
        total_conference_contents_links = []
        for conference in total_conference_list:
            links = conference.find_elements_by_tag_name("a")
            for link in links:
                if(link.text == "[contents]"):
                    conf_contents = link.get_attribute("href")
                    total_conference_contents_links.append(conf_contents)
        print total_conference_contents_links
        for contents_link in total_conference_contents_links:
            print contents_link
            ...
            try:
                ...
                print e
                external_links = ""
            context = document_list[0].find_elements_by_css_selector("span[itemprop='name']")[0].text
            for document_element in document_list[1:]:
                ...
                with open('../../../data/dblp_output.csv', 'a') as file:
                    file.write(line)
                    file.close()
    except Exception as e:
        print "Error at "+href
